...
void instance_sunwell_plateau::OnObjectCreate(GameObject* go)
{
    switch (go->GetEntry())
    {
        ...
        case GO_ICE_BARRIER: {
            go->GetVisibilityData().SetVisibilityDistanceOverride(
                VisibilityDistanceType::Infinite);
...
{
    WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_FELMYST, aMadrigosaLoc[0].x, aMadrigosaLoc[0].y, aMadrigosaLoc[0].z, aMadrigosaLoc[0].o, TEMPSPAWN_MANUAL_DESPAWN, 0, true), instance);
}
